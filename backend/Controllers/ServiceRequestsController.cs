...
public class ServiceRequestsController : ControllerBase
{
    ...
    public async Task<ActionResult<ServiceRequestDto>> Complete(int requestId, [FromBody] ServiceRequestCompletionDto completion)
    {
        var result = await _service.CompleteRequestAsync(requestId, completion.Comments);
        return Ok(result);
    }

    [HttpPost("tasks/{taskId}/complete")]
    public async Task<ActionResult<ServiceRequestDto>> CompleteTask(int taskId, [FromBody] ServiceRequestCompletionDto completion)
    {
        var result = await _service.CompleteTaskAsync(taskId, completion.Comments);
        return Ok(result);
    }
    ...
    public async Task<ActionResult<List<ServiceRequestDto>>> GetQueue()
    {
        ...
        {
            Console.WriteLine($"Queue Item: {req.RequestNumber}, Stage: {req.WorkflowStage}");
        }
        return Ok(queue);
    }
