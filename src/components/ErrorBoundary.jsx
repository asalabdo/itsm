...
const ErrorBoundaryContent = ({ hasError, children }) => {
      ...
      <div className="min-h-screen flex items-center justify-center bg-background text-foreground">
        <div className="text-center p-8 max-w-md">
          ...
          <div className="flex flex-col gap-1 text-center">
            <h1 className="text-2xl font-medium text-foreground">{t('somethingWentWrong', 'Something went wrong')}</h1>
            <p className="text-muted-foreground text-base w w-8/12 mx-auto">{t('unexpectedError', 'We encountered an unexpected error while processing your request.')}</p>
          </div>
          <div className="flex justify-center items-center mt-6">
            <button
              ...
              className="bg-primary hover:opacity-90 text-primary-foreground font-medium py-2 px-4 rounded flex items-center gap-2 transition-colors duration-200 shadow-sm"
            >
...
class ErrorBoundary extends React.Component {
  ...
  render() {
    return (
      <ErrorBoundaryContent hasError={this.state?.hasError}>
        {this.props?.children}
