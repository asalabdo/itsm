...
const SearchBar = () => {
  ...
  const quickFilters = [
    { label: 'My Open Tickets', icon: 'Ticket', path: '/agent-dashboard' },
    { label: 'High Priority', icon: 'AlertCircle', path: '/agent-dashboard' },
    { label: 'Unassigned', icon: 'UserX', path: '/agent-dashboard' },
    { label: 'Overdue', icon: 'Clock', path: '/agent-dashboard' },
  ];
  ...
  const searchResults = [
    {
      ...
      type: 'ticket',
      title: 'Ticket #4521',
      subtitle: 'Login issues with mobile app',
      status: 'urgent',
    },
    {
      id: 2,
  ...
  const handleQuickFilterClick = (path) => {
    navigate(path);
    setIsExpanded(false);
    setShowResults(false);
  };
  ...
  return (
    <div className="relative" ref={searchRef}>
      {!isExpanded ? (
        ...
        <div className="fixed md:absolute left-0 md:left-auto right-0 top-0 md:top-auto w-full md:w-96 bg-popover border border-border rounded-none md:rounded-md shadow-elevation-4 z-dropdown">
            ...
            {showResults && searchQuery ? (
              <div className="py-2">
                ...
                {searchResults?.map((result) => (
                  ...
                  >
                    <Icon
                      name={result?.type === 'ticket' ? 'Ticket' : 'User'}
                      size={18}
                    ...
                    <div className="flex-1 text-left min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground truncate caption">
                        {result?.subtitle}
                      </div>
                    </div>
                    {result?.status && (
                      <span
                        className={`px-2 py-0.5 text-xs rounded caption ${
                          result?.status === 'urgent' ?'bg-error/10 text-error' :'bg-warning/10 text-warning'
                        }`}
                      >
                        {result?.status}
                      </span>
                ...
                <div className="py-2 border-t border-border">
                  ...
                  {quickFilters?.map((filter, index) => (
                    <button
                      key={index}
                      className="w-full flex items-center gap-3 px-3 py-2 hover:bg-muted transition-smooth"
                      onClick={() => handleQuickFilterClick(filter?.path)}
                    >
                      <Icon
                        name={filter?.icon}
                        size={18}
