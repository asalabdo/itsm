...
const ApprovalHistoryPanel = () => {
  ...
  const filteredHistory = useMemo(() => {
    return approvalHistory.filter((item) => {
      ...
      const createdAt = item.decisionAt ? new Date(item.decisionAt) : null;
      const now = new Date();
      let matchesTime = true;

      if (timeFilter === 'today') {
        matchesTime = createdAt ? createdAt.toDateString() === now.toDateString() : true;
      } else if (timeFilter === 'week') {
        matchesTime = createdAt ? (now - createdAt) <= 7 * 24 * 60 * 60 * 1000 : true;
      } else if (timeFilter === 'month') {
        matchesTime = createdAt ? (now - createdAt) <= 30 * 24 * 60 * 60 * 1000 : true;
      }

      return matchesStatus && matchesTime;
    });
  }, [approvalHistory, statusFilter, timeFilter]);
