...
const ApprovalRequestCard = ({ request, onSelect, onOpen, isSelected, onApprove, onDeny }) => {
  ...
  const getSLAStatus = (hoursRemaining) => {
    const numeric = Number(hoursRemaining);
    if (!Number.isFinite(numeric)) return { color: 'text-muted-foreground', icon: 'Clock', label: 'Not set' };
    if (numeric < 0) return { color: 'text-error', icon: 'AlertTriangle', label: 'Overdue' };
    if (numeric < 4) return { color: 'text-warning', icon: 'Clock', label: 'Urgent' };
    ...
    >
      ...
      <div className="space-y-2 mb-3">
        <div className="flex items-center gap-2 text-xs md:text-sm">
          ...
          <span className="text-foreground truncate">{request?.requester || 'Unknown requester'}</span>
        </div>
        <div className="flex items-center gap-2 text-xs md:text-sm">
          <Icon name="Building2" size={14} className="text-muted-foreground flex-shrink-0" />
          <span className="text-muted-foreground truncate">{request?.department || 'Unassigned'}</span>
        </div>
        <div className="flex items-center gap-2 text-xs md:text-sm">
          <Icon name="Calendar" size={14} className="text-muted-foreground flex-shrink-0" />
          <span className="text-muted-foreground">{request?.submissionDate || 'Unknown'}</span>
        </div>
        <div className="flex items-center gap-2 text-xs md:text-sm">
      ...
      {request?.value && (
        <div className="flex items-center gap-2 mb-3 p-2 bg-muted rounded">
          <span className="text-xs font-bold text-muted-foreground">ريال</span>
