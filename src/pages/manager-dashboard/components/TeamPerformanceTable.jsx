  ...
  return (
    <div className="bg-card border border-border rounded-lg shadow-elevation-1">
      <div className="p-4 md:p-6 border-b border-border">
        <h2 className="text-lg md:text-xl font-semibold text-foreground">Team Performance</h2>
        <p className="text-sm text-muted-foreground mt-1 caption">Individual agent metrics and workload status</p>
      </div>
                ...
                <td className="p-3 md:p-4">
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-muted rounded-full h-2 overflow-hidden min-w-[60px] md:min-w-[80px]">
                      <div 
                        className={`h-full transition-smooth ${getWorkloadColor(agent?.workload)}`}
      ...
      <div className="p-4 border-t border-border flex flex-col md:flex-row items-center justify-between gap-4">
        <p className="text-sm text-muted-foreground caption">
          Showing {startIndex + 1}-{Math.min(startIndex + itemsPerPage, sortedData?.length)} of {sortedData?.length} agents
        </p>
        <div className="flex items-center gap-2">
          <button
