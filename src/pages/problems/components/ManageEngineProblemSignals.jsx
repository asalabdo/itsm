      ...
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          <div className="flex items-center gap-2 text-primary mb-1">
            ...
            <h2 className="text-lg font-semibold text-foreground">ManageEngine Problem Signals</h2>
          </div>
          <p className="text-sm text-muted-foreground">
            External alert and request signals that can help confirm recurring patterns before creating root-cause work.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          <div className="rounded-xl border border-border p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-medium text-foreground">OpManager Alerts</h3>
            </div>
            <div className="space-y-3">
              {alerts.length === 0 ? (
                <p className="text-sm text-muted-foreground">No active external alerts are available.</p>
              ) : (
                alerts.map((alert) => (
                  <div key={`${alert.source}-${alert.externalId}`} className="rounded-lg bg-muted/30 p-3">
                    ...
                    <div className="text-xs text-muted-foreground mt-1">{alert.status || 'Active'}</div>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="rounded-xl border border-border p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-medium text-foreground">ServiceDesk Requests</h3>
            </div>
            <div className="space-y-3">
              {requests.length === 0 ? (
                <p className="text-sm text-muted-foreground">No linked external requests are available.</p>
              ) : (
