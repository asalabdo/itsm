...
const CreateTicketModal = ({ isOpen, onClose, onSubmit, currentUser }) => {
  ...
  // Mock current user's assigned assets
  const mockUserAssets = [
    ...
    },
    {
      ...
      status: 'active',
      serialNumber: 'SN-IP15PM-2024-003'
    }
  ];
  ...
  const validateAssetOwnership = () => {
    ...
    // Simulate API call to check user's assigned assets
    setTimeout(() => {
      const userAssets = mockUserAssets?.filter(asset => asset?.status === 'active');
      ...
      if (userAssets?.length > 0) {
        ...
        setAssetValidation({
          ...
          hasAssets: false,
          assets: [],
          message: 'No assigned assets found. Please contact your administrator.'
        });
      }
    }, 800);
  };
